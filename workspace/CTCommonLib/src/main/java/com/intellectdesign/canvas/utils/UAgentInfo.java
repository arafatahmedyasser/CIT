...
public class UAgentInfo
{
	...
	/**
	 * Detects if the current device is a (small-ish) Android OS-based device used for calling and/or multi-media (like
	 * a Samsung Galaxy Player). Google says these devices will have 'Android' AND 'mobile' in user agent. Ignores
	...
	public boolean detectBlackBerryHigh()
	{
		...
		if (detectBlackBerry())
		{
			if (detectBlackBerryTouch() || userAgent.indexOf(deviceBBBold) != -1
	...
	public boolean detectSmartphone()
	{
		return (isIphone || isAndroidPhone || isTierIphone || detectS60OssBrowser() || detectSymbianOS()
				|| detectWindowsMobile() || detectWindowsPhone7() || detectBlackBerry() || detectPalmWebOS()
	 ...
	 * web content. Includes iPhone, iPod Touch, Android, Windows Phone 7, Palm WebOS, etc.
	 * 
	 * @return detection of any device in the iPhone/Android/WP7/WebOS Tier
	 */
	public boolean detectTierIphone()
	{
		if (isIphone || isAndroidPhone || (detectBlackBerryWebKit() && detectBlackBerryTouch())
				|| detectWindowsPhone7() || detectPalmWebOS() || detectGarminNuvifone())
		{
			return true;
		}
		return false;
