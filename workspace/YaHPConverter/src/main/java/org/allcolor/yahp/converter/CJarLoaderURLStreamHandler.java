...
public class CJarLoaderURLStreamHandler extends URLStreamHandler
{
	...
	public static class CGCCleaner
	{
		 ...
		 * 
		 * @param name preload entry map key
		 * @param ref The object to encapsulate
		 */
	 ...
	 * 
	 * @param path path to the class inside the jar
	 * @param array byte representation
	 */
	public void addClassPreload(final String path, final byte array[])
	{
		try
		{
			this.preload.put(path, new SoftReference(new CGCCleaner(path, array)));
