...
/**
 * This is a simple mutex (Mutual Exclusion) class, which permits to synchronize Thread access to some parts of the
 * code. Example usage : CMutex m = new CMutex(); private void mymethod() { m.acquire(); try { do something... } finally
 * { m.release(); } }
 * 
 * @author Quentin Anciaux
 * @version 0.94
...
public final class CMutex
{
	 ...
	 * 
	 * @param mutex the mutex array to acquire
	 */
	 ...
	 * 
	 * @param mutex the mutex array to release
	 */
	 ...
	 * Acquires this mutex
	 * 
	 * @param timeout maximum time to wait for acquiring
	 * 
	 * @throws RuntimeException if timeout is reached and no acquire was possible.
	 */
	public final void acquire(long timeout) throws RuntimeException
	{
		synchronized (this.LOCK)
		{
			...
			while (this.owner != null)
			{
				...
				if ((this.owner != null) && (timeout != -1) && ((end - start) >= timeout))
				{
	 ...
	 * Releases the mutex
	 * 
	 * @throws IllegalMonitorStateException If currentThread is not the owner.
	 */
	public final void release()
	{
		synchronized (this.LOCK)
		{
			...
			throw new IllegalMonitorStateException(Thread.currentThread().getName() + " is not the owner of the lock. "
					+ ((this.owner != null) ? ((Thread) this.owner.get()).getName() : "nobody") + " is the owner.");
		}
	}
}