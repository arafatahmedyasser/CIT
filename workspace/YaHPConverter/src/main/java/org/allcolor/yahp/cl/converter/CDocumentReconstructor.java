...
/**
 * Use iText to construct a complete pdf document from differents pdf parts. Apply configured security policies on the
 * resulting pdf.
 * 
 * @author Quentin Anciaux
 * @version 1.2.20b
...
public class CDocumentReconstructor
{
	...
	{
		int securityType = 0;
		securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_PRINTING)) ? (securityType | PdfWriter.ALLOW_PRINTING)
				: securityType;
		securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_MODIFY_CONTENTS)) ? (securityType | PdfWriter.ALLOW_MODIFY_CONTENTS)
				: securityType;
		securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_COPY)) ? (securityType | PdfWriter.ALLOW_COPY)
				: securityType;
		securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_MODIFT_ANNOTATIONS)) ? (securityType | PdfWriter.ALLOW_MODIFY_ANNOTATIONS)
				: securityType;
		securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_FILLIN)) ? (securityType | PdfWriter.ALLOW_FILL_IN)
				: securityType;
		securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_SCREEN_READERS)) ? (securityType | PdfWriter.ALLOW_SCREENREADERS)
				: securityType;
		securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_ASSEMBLY)) ? (securityType | PdfWriter.ALLOW_ASSEMBLY)
				: securityType;
		securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_DEGRADED_PRINTING)) ? (securityType | PdfWriter.ALLOW_DEGRADED_PRINTING)
				: securityType;
	 ...
	 * 
	 * @throws CConvertException if an error occured while reconstruct.
	 */
			...
			throws CConvertException
	{
		...
		try
		{
			...
			out.close();
			{
				...
				while (i < n)
				{
					...
					for (final Iterator it = hf.iterator(); it.hasNext();)
					{
						...
						{
							trans.transform(new ByteArrayInputStream(text.getBytes("utf-8")), base_url, new PageSize(
									dsize[0] - (dmargin[0] + dmargin[1]), dmargin[3]), new ArrayList(), properties,
									bbout);
						} else if (chf.getType().equals(CHeaderFooter.FOOTER))
						{
							trans.transform(new ByteArrayInputStream(text.getBytes("utf-8")), base_url, new PageSize(
									dsize[0] - (dmargin[0] + dmargin[1]), dmargin[2]), new ArrayList(), properties,
									bbout);
						}
			...
			if (signed)
			{
				...
				final KeyStore ks = selfSigned ? KeyStore.getInstance(KeyStore.getDefaultType()) : KeyStore
						.getInstance("pkcs12");
		...
		{
			throw new CConvertException("ERROR: An Exception occured while reconstructing the pdf document: "
					+ e.getMessage(), e);
		} // end catch
		finally
		{
