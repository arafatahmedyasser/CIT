...
public class CBASE64Codec
{
	...
	private static int[] decode(byte[] buffer)
	{
		...
		return new int[]
		{ ((bufferValue & 0x00FC0000) >>> 18), ((bufferValue & 0x0003F000) >>> 12), ((bufferValue & 0x00000FC0) >>> 6),
				(bufferValue & 0x0000003F) };
	}

	private static int[] decode(char[] buffer)
	{
		return new int[]
		{ CBASE64Codec.B64ARRAY.indexOf(buffer[0]), CBASE64Codec.B64ARRAY.indexOf(buffer[1]),
				CBASE64Codec.B64ARRAY.indexOf(buffer[2]), CBASE64Codec.B64ARRAY.indexOf(buffer[3]) };
	}
