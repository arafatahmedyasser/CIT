...
/**
 * This class handle the {'http://www.allcolor.org/xmlns/yahp','pb'} tag and cut document in multiples documents
 * according to it.
 * 
 * @author Quentin Anciaux
 * @version 0.94
...
public class CDocumentCut
{
	...
	public static DocumentAndSize[] cut(final Document doc, final PageSize size)
	{
		...
		if (nl.getLength() == 0)
		{
			return new DocumentAndSize[]
			{ new DocumentAndSize(doc, size) };
		} // end if
		...
		for (int i = 0; i < pbdocs.length; i++)
		{
			...
			CNamespace xmlnsdef = new CNamespace((pbend != null) ? pbend.getPrefix() : pbstart.getPrefix(),
			...
			if (pbend == null)
			{
				...
				Element parentPb = (Element) ndoc.adoptNode(pbstart.getParentNode().cloneNode(false));
			...
			{
				Element parentPb = (Element) ndoc.adoptNode(pbend.getParentNode().cloneNode(false));
				...
				if (hasbeendes && pbstart != Sibling)
				{
					...
					((Element) c).setAttributeNS("http://www.w3.org/2000/xmlns/", "xmlns:yahp",
							"http://www.allcolor.org/xmlns/yahp");
							...
							if (!("body".equals(ppr.getNodeName())))
							{
								...
								while (!hasbeendes && (Sibling != null))
								{
									if (isDescendant(pbstart, Sibling))
									{
										...
										{
											Node c = ndoc.adoptNode(Sibling.cloneNode(true));
											Node pb = ((Element) c).getElementsByTagNameNS(
													"http://www.allcolor.org/xmlns/yahp", "pb").item(0);
											Node p = pb;
											Node pp = pb.getParentNode().getPreviousSibling();
	 ...
	 * @return true if n is a descendant of ref
	 */
	private static boolean isDescendant(final Node n, final Node ref)
	{
		if (ref == null)
		{
			return false;
	 ...
	 */
	private static void copyHeader(final Document doc, final Document ndoc)
	{
		...
		if (headnl.getLength() > 0)
		{
			...
			ndoc.getDocumentElement().insertBefore(ndoc.adoptNode(head.cloneNode(true)),
	...
	private static class PbDocument
	{
		 ...
		 */
		public PbDocument(final Element pbStart, final Element pbEnd)
		{
