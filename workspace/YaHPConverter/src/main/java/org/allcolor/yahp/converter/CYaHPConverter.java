...
public final class CYaHPConverter
{
	 ...
	 */
	public final void convertToPdf(final URL url, final IHtmlToPdfTransformer.PageSize size, final List hf,
			final OutputStream out, final Map fproperties) throws CConvertException
	{
		...
		try
		{
			Thread.currentThread().setContextClassLoader(
					new URLClassLoader(new URL[0], this.useClassLoader ? CClassLoader.getLoader("/main") : this
	 ...
	 * @param size PDF Page size
	 * @param hf header-footer list
	 * @param furlForBase base url of the document, mandatory, must end with a '/'
	 * @param out outputstream to render into
	 * @param fproperties properties map
	 * 
	 * @throws CConvertException if an unexpected error occurs
	 */
	public final void convertToPdf(final String content, final IHtmlToPdfTransformer.PageSize size, final List hf,
			final String furlForBase, final OutputStream out, final Map fproperties) throws CConvertException
	{
		...
		try
		{
			...
			{
				IHtmlToPdfTransformer transformer = getTransformer(properties);
				transformer.transform(new ByteArrayInputStream(content.getBytes("utf-8")), urlForBase, size, hf,
						properties, out);
			} catch (final CConvertException e)
			{
				throw e;
	...
	/**
	 * If instance count fall to 0, Delete the jar file from the temp directory, Destroy the classloader by calling the
	 * destroy method
	 * 
		...
		try
		{
			...
			if (url != null)
			{
				...
				try
				{
					...
					Method configure = pc.getDeclaredMethod("configure", new Class[]
					{ URL.class });
	 ...
	 * 
	 * @return a transformer corresponding to the criterias found in the properties map.
	 */
	private IHtmlToPdfTransformer getTransformer(final Map properties)
	{
		try
		{
			...
			{
				ClassLoader bootStrap = this.useClassLoader ? CClassLoader.getLoader("/main") : this.getClass()
						.getClassLoader();
				...
				try
				{
					...
					{
						Thread.currentThread().setContextClassLoader(
								this.useClassLoader ? CClassLoader.getRootLoader() : this.getClass().getClassLoader());
				...
				{
					e.printStackTrace();
					System.err.println("SEVERE: Error while getting transformer '" + rendererClassName + "' ! : "
							+ e.getMessage());
