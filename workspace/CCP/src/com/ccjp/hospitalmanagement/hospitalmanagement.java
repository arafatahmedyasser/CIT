		...
		try {
			if(f.exists()){
				...
				while((readLine=br.readLine())!=null){
					...
					if(adDate.after(disDate))throw new hospitalmanagementException("Invalid Admission date");
				...
				Collections.sort(hl, new Comparator<hospitalVO>() {
				...
				System.out.println("hl-----"+hl);
					...
					Set<Date> adDateLisr=new TreeSet<Date>();
					
					for(hospitalVO d:hl){
						String specialist=d.getMnrNo().split("-")[1];
						adDateLisr.add(d.getAdmissionDate());
						int ct=(specialist.equalsIgnoreCase("GEN"))?gen++:(specialist.equalsIgnoreCase("NEU"))?neu++:(specialist.equalsIgnoreCase("ENT"))?ent++:0;
					}
					System.out.println("gen -"+gen+"-neu-"+neu+"-ent-"+ent);
					System.out.println();
					SimpleDateFormat sd1=new SimpleDateFormat("E- dd MMM yyyy HH:MM:ss");
					...
					for(Date adDate1:adDateLisr){
						...
						System.out.println(sd1.format(adDate1)+"----"+hh1);
						System.out.println();
					}
