...
/**
 * The TextField class renders the Text Box that produce HTML pages
 * 
 * @version 1.0
 * 
 */
public class TextField implements Renderable
{
	 ...
	 * @param item
	 */
	public TextField(Map item)
	{
		this.item = item;
	}

	public TextField(FormExportModel formModel,Map itemMetadata){ 
	...
	private void renderValue(HtmlCanvas html) throws IOException {
		html.td(new HtmlAttributes().width("30%"))
         .div(new HtmlAttributes().class_("NormalTD_PDF REV_UNICODE")).content((String) item.get("screenViewData"))
     ._td();
