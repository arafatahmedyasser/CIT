 ...
 * @description This component is currently responsible Jquery Framework to rendered tabletop layout header.
 */
canvas.applnlayout.tabletop.header = Class({
    /**
     * @class "canvas.applnlayout.tabletop.header"
    ...
    getHeaderDOM: function() {
        ...
        var tmpLayer = new ct.lib.tmplLayer('al-tabletop-header.cttpl', componentJSON);
    ...
    applyTemplate: function(template, tmpClass) {
        ...
        $(this.parentElem).find('[data-item-id=ct_logo]').on('click', function() {
            ...
            if (home_action) {
                $("#FOOTER_DIV .ct-al__tabletop-each").removeClass('ct-al__tabletop-is_selected');
                if (!cbx.core.isEmpty(that.customJSON) && that.customJSON.isLandingPageRequired() == false) {
                    $("#FOOTER_DIV .ct-al__tabletop-each:first").addClass("ct-al__tabletop-is_selected");
                } else {
...
}, canvas.applnlayout.tabletop.header);
