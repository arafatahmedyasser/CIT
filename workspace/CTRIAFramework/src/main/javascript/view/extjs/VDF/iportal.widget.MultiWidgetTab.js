...
/*
 * @class MultiWidgetTab class provided a container to load all its child widgets into individual tabs
 */

iportal.widget.MultiWidgetTab = Ext.extend(Ext.TabPanel, {
	childWidgets : [],
	id : null,
	bundle : CRB.getFWBundleKey(),
	autoHeight : true,
	baseCls : 'appTab x-tab-panel',
	...
	initComponent : function ()
	{
		...
		iportal.widget.MultiWidgetTab.superclass.initComponent.apply(this);
	},
	/*
	...
	createItems : function ()
	{
		...
		for (var index = 0; index < this.childWidgets.length; index++)
		{
			...
			if(this.childWidgets[index].VIEWS_LIST.length>0){
				...
				if(lastState!=='NOT_FOUND' && cbx.isObject(lastState)){
					...
					for(viewNmIndex=0;viewNmIndex<this.childWidgets[index].VIEWS_LIST.length;viewNmIndex++){
						if(this.childWidgets[index].VIEWS_LIST[viewNmIndex].VIEW_ID===lastState._mvhOpt.sel_vw){
							displayName = (null == rbw[this.childWidgets[index].VIEWS_LIST[viewNmIndex].VIEW_DISPLAY_NM])
							? this.childWidgets[index].VIEWS_LIST[viewNmIndex].VIEW_DISPLAY_NM
							: rbw[this.childWidgets[index].VIEWS_LIST[viewNmIndex].VIEW_DISPLAY_NM]
						}
					}
				}
				else{
				...
				displayName = (null == this.rb[this.childWidgets[index].WIDGET_DISPLAY_NM])
							? this.childWidgets[index].WIDGET_DISPLAY_NM
							: this.rb[this.childWidgets[index].WIDGET_DISPLAY_NM]
			}
	...
	tabSelectionHandler : function (tabPanel)
	{
		try
		{
			...
			widget.mv.isLoadingToolsInside = false;
	...
	afterRender : function ()
	{
		...
		iportal.widget.MultiWidgetTab.superclass.afterRender.apply(this, arguments);
	},
