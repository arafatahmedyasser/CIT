...
Ext.ns('cbx.appstore');
/**
 * @class cbx.appstore.DataViewTransition
 * @extends Object
 
 * Transition plugin for DataViews (Modified the original version with some fixes)
 */
cbx.appstore.DataViewTransition = Ext.extend(Object, {
	...
	init: function(dataview) {
		...
		dataview.updateIndexes = dataview.updateIndexes.createSequence(function() {
			this.getTemplateTarget().select(this.itemSelector).each(function(element, composite, index) {
				element.id = element.dom.id = 
									String.format("{0}-{1}", dataview.id, store.getAt(index).get(idProperty));
			}, this);
		}, dataview);
		...
		var datachangefunction = function(store) {
			...
			if(Ext.fly(this.dataviewID + '-' + item.get(this.idProperty))){
				Ext.fly(this.dataviewID + '-' + item.get(this.idProperty)).ghost('b',{
					easing: 'easeOut',
					remove  : false,
		...
		//store.on('remove', datachangefunction,this);
		store.on('datachanged', datachangefunction,this);
	},
	...
	getRemoved: function(store) {
		...
		for (var id in this.cachedStoreData) {
			if (store.findExact(this.idProperty,id) == -1) removed.push(this.cachedStoreData[id]);
		}
...
Ext.reg('DataViewTransition', cbx.appstore.DataViewTransition);