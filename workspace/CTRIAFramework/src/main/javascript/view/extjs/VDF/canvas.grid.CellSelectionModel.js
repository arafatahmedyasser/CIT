...
canvas.grid.CellSelectionModel = function (config)
{
	 ...
	 * @param {canvas.grid.CellSelectionModel} this
	...
	canvas.grid.CellSelectionModel.superclass.constructor.call(this);
};

Ext.extend(canvas.grid.CellSelectionModel, Ext.grid.AbstractSelectionModel, {
	/**
	...
	hasNext : function ()
	{
		return this.last !== false
					&& ((this.last[0] + 1) < this.grid.store.getCount() || (this.last[1] + 1) < this.grid
	...
	selectCell : function (index, keepExisting, preventViewNotify)
	{
		...
		if (row < 0 || row >= this.grid.store.getCount())
			return;
		if (col < 0 || col >= this.grid.getColumnModel().getColumnCount())
			return;

		if (this.fireEvent("beforecellselect", this, index, keepExisting) !== false)
		{
			//----------------------- SRS - Starts to select multicell -----------------
			if (this.isSelected(index)){
				this.clearSelections();
			}
