...
Ext.ux.grid.filter.StringFilter = Ext.extend(Ext.ux.grid.filter.Filter, {
	...
	init : function(config) {
		...
		this.menu = new Ext.ux.menu.RangeMenuString(Ext.apply(config, {
			// pass along filter configs to the menu
			fieldCfg : this.fieldCfg || {},
			fieldCls : this.fieldCls,
			fields : this.fields || {},
			iconCls : this.iconCls,
			stringFilterField : true,
			lookupFilterWindow : null,
			...
			listeners : {
				'beforehide' : function() {
					if (this.enableMenu) {
						...
						if (this.lookupFilterWindow) {
							this.enableMenu = false;
							try {
								this.lookupFilterWindow.close();
							} catch (err) {
							}
						}
					}
