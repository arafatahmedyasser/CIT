...
Ext.ux.grid.LockingGridView = Ext.extend(Ext.grid.GridView, {
	 ...
	 * row heights aren't predictable.
	 */
    syncHeights: false,
    ...
    doRender : function(cs, rs, ds, startRow, colCount, stripe){
        ...
        for(var j = 0, len = rs.length; j < len; j++){
            ...
            for(var i = 0; i < colCount; i++){
                ...
                p.attr = p.cellAttr = '';
                p.value = c.renderer(r.data[c.name], p, r, rowIndex, i, ds,this.grid); // CT_RESPONSIVE
                p.style = c.style;
    ...
    afterRender : function(){
        ...
        // CHGCONTEXTCOUMN Starts
        /*
		 * Code added to the remove the extra line space for the locked and
		 * unlocked rows
		 */
        if(this.cm){
        	...
        	if(lockedCount==0){
                ...
                lockedWrap.addClass('no-lockedrows');
                var unLockedWrap = el.child('div.x-grid3-unlocked');
                unLockedWrap.addClass('unLockedrows');
        	}
        }
        // CHGCONTEXTCOUMN Ends
    ...
    afterRenderUI: function(){
        ...
        if(g.enableColumnMove){
        	...
        	/*
			 * updated for locking grid's action column or column not to be
			 * draggable. updated 07/06/2012// LG&AC-001 updated by Harriesh.v &
			 * Sudha.N CHG_CBT_Q1 STARTS
			 */
            	this.columnDrag.getDragData = function(e){
	            ...
	            this.columnDrop.onNodeDrop = function(n, dd, e, data) {
	                var h = data.header;
	                if(h != n){
	                    var cm = this.grid.colModel,
        ...
        if(g.enableHdMenu !== false){
            ...
            if(this.grid.enableColLock !== false){
                this.hmenu.add(// '-',
                    {itemId: 'lock', hidden :true,text: this.lockText, cls: 'xg-hmenu-lock'},
                    {itemId: 'unlock',hidden :true, text: this.unlockText, cls: 'xg-hmenu-unlock'}
                );
            }
    ...
    // CHG_SCROLL_ISSUE - Ends
});

Ext.ux.grid.LockingColumnModel = function(config) {
...
Ext.ux.grid.LockingColumnModel = Ext.extend(Ext.grid.ColumnModel, {
	...
	initComponent : function() {
		Ext.ux.grid.LockingColumnModel.initComponent.apply(this,
				arguments);
	},
	
	canLockChange : function(colIndex) {
		if (this.config[colIndex].lockfixed === true)
			return false;
		else
			return true;
	},	
