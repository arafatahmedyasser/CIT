...
Ext.ux.AddTabButton = (function() {
	function onTabPanelRender() {
		...
		new Ext.ToolTip({
			target : this.addTab,
			bodyCfg : {
				html : CRB.getFWBundle()['LBL_MANAGE_WS']
			}
		});
		this.addTab.on({
			mousedown : stopEvent,
	...
	function autoScrollTabs() {
		var scrollersVisible = (this.scrollLeft && this.scrollLeft.isVisible()), pos = this.tabPosition == 'top' ? 'header'
				: 'footer';
		if (scrollersVisible) {
	...
	/*
	 * function onAddTabClick() { this.setActiveTab(this.add(this.createTab?
