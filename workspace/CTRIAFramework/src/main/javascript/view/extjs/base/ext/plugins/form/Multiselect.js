...
Ext.ux.form.MultiSelect = Ext.extend(Ext.form.Field,  {
    ...
    onRender: function(ct, position){
        Ext.ux.form.MultiSelect.superclass.onRender.call(this, ct, position);
		        ...
		        /* Adding the multiselct internal configuration in the fieldset */
       
        var fs = this.fs = new Ext.Panel({
            height: this.height-60,//calculating the height of the fieldset after subtracting the title height of the panel.
            autoScroll:true           
        });
        
		        /*
				 * Panel configuration inside which each of the multiselect
				 * component(available list panel and selected list panel) will
				 * be rendered
				 */
       var mainPanel=new Ext.Panel({
        	...
        	renderTo: this.el,
        	autoHeight:true,
        	style: "padding:0;border:1px solid #B5B8C8",
        	items:[this.fs,
        	       {
              	 tbar:[{
              		 id:Ext.id(),
        ...
        this.view = new Ext.ListView({
            ...
            multiSelect: true,
            store: this.store,
            columns: [{ header: 'Value', dataIndex: this.displayField }],
            hideHeaders: true
        });
