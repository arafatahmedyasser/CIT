	...
	return {
			...
			if(vTypeRegistry.length==0){
				try{
					...
					if(this.validateConfiguration(name, value, mask,text,globalRe)==false){
						...
						this.addToRegistry(name,mask,text,value,globalRe);
		...
		validateConfiguration:function(name, value, mask,text,globalRe){
			if(Ext.isEmpty(value) || value==""){
				...
				return false;
			}else if(Ext.isEmpty(name) || name==""){
				LOGGER.error("There is a configuration issue in the name of the vtype you configured");
				return false;
			}else if(name=='email' || name=='url' || name=='alpha' || name=='alphanum'){
				LOGGER.error("The vtypes are already configured as a prt of Ext.form.vTypes");
				return false;			
			}else if(Ext.isEmpty(mask) || mask==""){
					LOGGER.error("There is a configuration issue in the mask of the vtype you configured");
					return false;
			}
			else if(Ext.isEmpty(text) || text==""){
		...
		/**
		 * @Method(Private):The method will be used to register the vtype as a
		 *                      part of Ext.form.vTypes.Expected to be called by
		 *                      setVtype method
		 */
		utilityToSetVtypes:function(name, value, mask,text){
			
			Ext.form.VTypes[name+'Val'] = value;
			Ext.form.VTypes[name+'Mask'] = mask
			Ext.form.VTypes[name+'Text'] = text
			Ext.form.VTypes[name] = function(v){
			return Ext.form.VTypes[name+'Val'].test(v);
			}
		},
		/**
		 ...
		 */
		addToRegistry:function(name,mask,text,value,globalRe){
			vTypeRegistry.push({
				'name' : name,
				'mask' : mask,
