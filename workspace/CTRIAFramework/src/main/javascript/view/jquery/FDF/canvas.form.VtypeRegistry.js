...
canvas.form.vTypeRegistry = function (){
	...
	return {
		 ...
		 *</PRE>
		 */
		setVtype : function (name, value, mask,text){
			if(vTypeRegistry.length==0){
				try{
					if(this.validateConfiguration(name, value, mask,text)==false){
						return;
					}
					else if(this.validateConfiguration(name, value, mask,text)==true){
						this.addToRegistry(name,mask,text);
		 ...
		 * </PRE>
		 */
		validateConfiguration:function(name, value, mask,text){
		 ...
		 */
		addToRegistry:function(name,mask,text){
			vTypeRegistry.push({
				'name' : name,
				'mask' : mask,
				'text' :text
