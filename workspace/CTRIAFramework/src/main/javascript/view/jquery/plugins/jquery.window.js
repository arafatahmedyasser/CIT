...
$.Window = (function()  {
	...
	function getCssStyleByDock(parent, miniIndex) {
		...
		if( setting.dockArea != null ) {
			...
			lng = parent.get(0)._minWinData['long'];
		} else if( parent != null ) {
		...
		if( setting.dock == 'left' || setting.dock == 'right' ) {
			...
			targetCss.height = lng - 1;
			targetCss.top = miniIndex * lng;
			...
			targetCss.width = lng - 1;
			targetCss.height = narrow;
			targetCss.left = miniIndex * lng;
			if( setting.dock == 'top' ) {
				if( parent != null ) {
					targetCss.top = cpos.top;
					targetCss.left += cpos.left;
