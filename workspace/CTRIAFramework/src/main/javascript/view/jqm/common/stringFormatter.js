...
//
cbx.amtutil.numutil = function(){
	// constants used for number manipulation.
	var COMMA_DELIMITER = ','; 	
	var PERIOD = '.'; 	
	var ZERO_NUMERAL = '0'; 	
	var ONE_NUMERAL = '1'; 	
	...
	var formatter_instance = cbx.amtutil.stringnumformatter();
	return {
		// The method to add 2 string numbers. This is the method 
		// to be used by the clients of this class.
		// @param a : first number to be added
		// @param b : second number to be added
		// Both the parameters must be positive numbers
		add : function(a,b){
			if((a.indexOf(PERIOD) !== -1) || (b.indexOf(PERIOD) !== -1)){
				...
				if(partialSum.length === aCompArr.length){
					...
					//----------------
					// return the integer part and decimal part tied together with a period
					// incase decimals doesnt added up with a carry.
					if(decimalRemainder === 0)
		...
		// If that is not the case, padding zeroes will be added to the humble 
		// array.
		normalizeFraction : function(a,b){
			var resArray = [];			
			// 3 possible cases has been checked
			if(a.length === b.length){
				...
				// repeat will insert as many 0's as mentioned in the 
				// arguments column
		...
		// big numbers. The method is used as a utility for some other
		// methods
		min : function(a,b){
			r = ( a >= b ) ? b : a;
			return r;
		...
		_sub : function(a,b){
			var resArr = [];
			// convert both string numbers to number array
		...
		// This is a utility function which omits the leading zeroes from a 
		// string number
		basicFormatter : function(s){
			...
			if(intS.startsWith(ZERO_NUMERAL) && intS.length!== 1){
				// recursive call with omitted zero
		...
		europeanFormatter : function(s,sd){
			...
			var intS = s.substring(0,s.indexOf(PERIOD));	
			// give a default value for no fo significant digits to be included
			if(isNaN(sd)) { sd = 2; }
			var decS = s.substring(s.indexOf(PERIOD)+1,s.length);	
		...
		indianFormatter : function(s,sd){
			...
			var intArr = this.toNumArray(intS).reverse();
			intLen = intArr.length;
			// If integer part is less than 1000, return same
		...
		*/
		getNeededWidthNoEl : function(value,metadata){
			...
			var neededWidth = value.visualLength();	
			return neededWidth;
		},
		/**
		...
		*/
		getStringForWidth : function(val,valWidth,metadata){
			...
			for(var index = 0; index < val.length; index++){
				var tempVal=val.substring(0,index);
				var tempWidth =tempVal.visualLength();	
...
// use this factory to instantiate StringNumber object
// eg: var StringNumber = cbx.amtutil.stringnumber.getInstance();
// That will return a singleton instance of StringNumber to be used
// for calculations
cbx.amtutil.stringnumber = function(){
	var _stringnumInstance = null;
	return {
      getInstance : function(){
					 if(_stringnumInstance === null){
					    _stringnumInstance = cbx.amtutil.numutil();
					 }
...
cbx.amtutil.stringnumformatter = function(){
	// constants used for number manipulation.
	var COMMA_DELIMITER = ','; 	
	var PERIOD = '.'; 	
	var ZERO_NUMERAL = '0'; 		
	...
	return {		
		...
		euroFormatter : function(s,sd){
			...
			for(var index = 0; index< intLen; index++){
				if(index % 3 === 0 && index !== 0){
					resultArr.push(PERIOD); // change for swap
					resultArr.push(intArr[index]);
					continue;
				}
				resultArr.push(intArr[index]);
			...
			return resultArr.reverse().join(EMPTY_STRING).concat(COMMA_DELIMITER).concat(decS); // change for swap
		},
		// accepts a number in string form and formats the number in
		// european format.
		// @param s : String number to be formatted
