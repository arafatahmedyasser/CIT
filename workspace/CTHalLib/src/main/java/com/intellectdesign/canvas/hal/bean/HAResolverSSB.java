...
import com.intellectdesign.canvas.hal.HALException;
import com.intellectdesign.canvas.hal.HARequest;
import com.intellectdesign.canvas.hal.HAResponse;
import com.intellectdesign.canvas.hal.ICommunicator;
import com.intellectdesign.canvas.logger.Logger;

/**
 * 
 * The <code>HAResolverSSB</code> processes request and sends request to <code> ICommunicator </code>
 * 
 * @see com.intellectdesign.canvas.hal.ICommunicator
 * @see com.intellectdesign.canvas.hal.HACommunicator
 * 
 * @version 1.0
 */
public class HAResolverSSB implements SessionBean
{

	/**
	 * Internal constant for serialization purposes
	 */
	private static final long serialVersionUID = 2765062367219951518L;

	/**
	 * Session Context Objext
	 */
	...
	/** Creates a new instance of HAResolverSSB */
	public HAResolverSSB()
	{
	}
	...
	{
		String methodName = " [HAResolverSSB.ejbCreate()] ";
		logger = Logger.getLogger(HAResolverSSB.class);
	...
	{
		// Reset the logger instance just in case
		logger = Logger.getLogger(HAResolverSSB.class);
	}
	...
	/**
	 * Sets SessionContext Object
	 * 
	 * @param sessionContext
	 * @throws EJBException
	 * @throws RemoteException
	 */
	public void setSessionContext(SessionContext sessionContext) throws javax.ejb.EJBException,
	...
	/**
	 * Process method for Remote Interface. This method takes Request in HashMap and sends Response in HashMap. This
	 * method internally calls process(HARequest)
	 * 
	 * @param req HashMap
	 * @return HashMap
	 */
	public HashMap process(HashMap req)
	{
		...
		if (status != InfraConstants.SUCCESS)
		{
	...
	/**
	 * process method for Local Interface. This method takes Request as HARequest Object and sends Response in
	 * HAResponse Object. This method resolves the Communicator defined in Database Table and calls respective methods
	 * based on Sync/Async and Intellect or Non-Intellect.
	 * 
	 * @param haReq HARequest
	 * @return HAResponse
	 */
	public HAResponse process(HARequest haReq)
	{
		...
		if (haResp.getStatus() != InfraConstants.SUCCESS)
		{
	...
	/**
	 * Method for pocess the user request by passing HARequest as parameter.
	 * 
	 * @param haReq
	 * @return HARespose
	 */
			...
			} else
			{
				...
				} else
				{
					...
					} else if (val.trim().equals("F"))
					{
						...
						status = 0;
                    }else if (  val.trim().equals( "S" )) {
                        logger.ctdebug("CTHAL00054", methodName);
                        haResp = communicator.sendToSR(haReq, appidConfHM);
                        status = 0;
                    }else if (  val.trim().equals( "D" )) {		
                        logger.ctdebug("CTHAL00055", methodName);
                        haResp = communicator.sendToDB(haReq, appidConfHM);
                        status = 0;
